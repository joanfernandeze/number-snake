...
- [ ] **Step 2: Make sure git has an identity (local to this repo)**
...
- [ ] **Step 1: Add the INPUT constants**
...
- [ ] **Step 2: Update the existing snake tests to the new shape and add queue tests**
...
- [ ] **Step 4: Implement the queue in `src/snake.js`**
...
Run: `node --test`
Expected: all pass (38 tests: 31 previous + 7 new).
...
- [ ] **Step 5: Wire the gate minimally so the game still runs**
...
- [ ] **Step 5: Let JS size the canvas — edit `style.css`**
...
- [ ] **Step 7: Run all tests, then verify manually in the browser**

Run: `node --test`
Expected: all pass (51 tests).
...
- [ ] **Step 4: Implement `src/telemetry.js`**
...
- [ ] **Step 6: Hook telemetry into `src/main.js`**
...
- [ ] **Step 6: Rewrite `tools/simulate.js` with CLI overrides and length-at-death**
...
- [ ] **Step 3: Tick the checkboxes in this plan and commit**
...
- **Placeholder scan:** every code step contains complete code; every run step names the command and the expected outcome; test counts are stated per task (31 → 38 → 39 → 42 → 48 → 51 → 56 → 57).
